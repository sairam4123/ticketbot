...
class Tickets(commands.Cog):
    ...
    async def ticket(self,ctx):
        if discord.utils.get(ctx.guild.text_channels, name={ctx.author.name}-{ctx.author.discriminator}):
            #                                               ^^^^^^^^^^^^^^^ if the author has " " it will automatically replace " " to "-".
            #                                                                ^^^^^^^^^^^^^^^^^^^^^^^^^^ it would be nice if you made it directly.
            return await ctx.send(f"{ctx.author.mention} You already have a ticket!")
        overwrites = {
        ...
        channel = await ctx.guild.create_text_channel(name=args, overwrites=overwrites, topic=f"Ticket Owner: {ctx.author}\nTicket Owner ID: {ctx.author.id}\nTicketPro by editid#6714", slowmode_delay=1)
        ...
        try:
            if not ctx.channel.topic.endswith("TicketPro by editid#6714"):
                ...
                try:
                    ...
                    await ctx.send("I am not able delete this channel, please make sure that I have all necessary permissions")
            ...
            if reaction.emoji == "\U0001f3ab":
                if channel.topic.endswith("TicketPro by editid#6714"):
                    try:
                        ...
                        await channel.send("Hey! I don't have the permissions to delete this channel!")
            elif reaction.emoji == "\U000023f1":
                if channel.topic.endswith("TicketPro by editid#6714"):
                    try:
                        await channel.edit(slowmode_delay=0)
                    except:
                        await channel.send("Hey! I don't have the permissions to edit the slowmode!")
